...
# pku_exam_plus（拓展版，2026-08-09）

baseline 已满足题面两场景 × 五平台，冻结为 `baseline v1.0`（git tag）。
本目录在其之上做**任务拓展、稳定/速度优化、界面美化**，不回改 baseline 行为。
四条拓展主线全部落地并在 CPU / GPU 两档实测通过。

## 拓展一：场景三 —— 跨平台关键词搜索

题面只要求「榜单前 N」和「某创作者今日」两个场景。真实的舆情/选题需求里，
「给一个关键词，跨平台把最热的几条拉回来一起看」同样高频，故新增为场景三。

- `discovery/base.py` 加抽象 `search_videos(keyword, limit)`，五平台各自实现：
  - B 站：WBI 签名的 `/x/web-interface/wbi/search/type`，按综合热度排序。
  - 抖音 / 快手 / 微博 / 小红书：复用 MediaCrawler 各自的关键词搜索接口
    （`search_info_by_keyword` / `_v2` / `get_note_by_keyword`）。
- `orchestrator.run_search(keyword, limit)`：单平台搜索→下载→理解一条龙。
- `cli.py search --keyword 词 --platforms bili,xhs,wb`：多平台串行、重后端只加载一次，
  跨平台结果汇总后统一生成一次总览。
- `fetch_local.py` 泛化：新增 `--keyword`，且**支持 B 站**（B 站免登录、不开浏览器），
  让「本机采集下载 → 服务器理解」这条混合路对搜索场景同样可用。

**实测（混合路，B 站关键词「人工智能」）**：本机搜索发现 5 条、下载 2 条 →
传服务器 `understand.py --profile gpu --digest` → 2/2 成功、13.0s，
生成跨视频总览「AI 教程与 AI 搞笑视频并存」，并正确点出优先观看项。

## 拓展二：批次总览（cross-video digest）

单条摘要之外，整批跑完再让 LLM 归纳一层「这批视频整体在讲什么」：
提炼总标题、若干主题聚类（含归属视频）、跨视频观察、优先观看项与理由。

- `summarize/base.py` 加通用 `chat_json`（自由对话要 JSON），`openai_compat.py` 落实现。
- `summarize/digest.py`：把各条已归纳结果渲染成输入，调 `chat_json` 产出 `Digest`。
- 编排器 `_maybe_digest`：`run_ranking / run_creator / run_prefetched / run_search`
  收尾统一触发（`PipelineConfig.enable_digest`）。总览失败不影响主结果。

## 拓展三：报告导出（可分享 HTML / Markdown）

- `report.py`：`render_markdown` / `render_html` 把一次 run（含总览、每条摘要、
  转写/OCR 依据、各阶段耗时）渲染成自包含文档。HTML 无外链、可直接发给非技术同学。
- 入口三处：CLI `report <run_id> -f html|md`；Web `GET /api/history/{id}/report.{fmt}`；
  `understand.py --report path`（本次实测直接产出）。

## 拓展四：稳定性 & 速度

- **fast 模式**（`--fast` / `PipelineConfig.skip_ocr_if_transcript_chars=200`）：
  转写已足够充分（≥200 字）时跳过抽帧与 OCR。实测视频①转写 202 字→跳过 OCR，
  视频②转写 15 字→仍走 OCR 兜底，行为精确符合预期。
- **B 站 v_voucher 软风控修复**：搜索/签名接口除 412 硬拦截外，还有一种
  HTTP 200 + code 0 但 `data` 只回一个 `v_voucher` 兑换券的「假装没搜到」软风控。
  `_get_signed_json` 现在识别它并按同样的退避+换匿名身份重试（本地已复现修复）。
- 沿用 baseline 的 prefix-caching（vLLM）、分资源限流并发、单条失败隔离。

## 拓展五：新版 Web 界面

`web/static/index.html` 重做为 Forest 深色主题：运行配置区（含 search 模式与
关键词、fast/digest 开关）、实时流水线 runline、结果卡片墙（阶段 chip）、
批次总览面板、历史列表与一键报告导出。后端 `VSPIDER_WEB_BROWSER=1` 时放开
本机浏览器平台直连（本机有登录态 + 干净 IP）。

## CPU / GPU 端到端校验（同一 dy 批次，2 条，做 GPU vs CPU 对照）

| 档位 | 归纳后端 | 结果 | 墙钟 | 总览 | 报告 | fast 跳 OCR |
| --- | --- | --- | --- | --- | --- | --- |
| GPU | vLLM Qwen3-8B | 2/2 | 20.4s | ✓ | ✓ HTML | 命中（202 字那条） |
| CPU | llama.cpp Qwen2.5-3B | 2/2 | 157.6s | ✓ | ✓ HTML | 命中 |

两档均：CPU 跑 ASR（12 GB 卡 vLLM 独占显存）、结果入 SQLite、导出自包含 HTML 报告。
GPU 档归纳仅 1.73s，CPU 档单条归纳 60~77s（Qwen2.5-3B Q4 在 12 vCPU 上的正常速度）。

## 登录态与环境现状（本次核对）

- 四浏览器平台**真实取数 4/4 可用**（`verify_all.py dy ks wb xhs`），无需重新扫码。
- 本机是「瘦采集端」：无 ffmpeg、无 funasr/torch、yt-dlp 仅作为模块存在。
  因此重计算（ASR/OCR/LLM）全部在服务器；B 站本机采集只能拿单流（无音轨时
  自动降级为 OCR 理解，已实测）。
- 服务器 IP 已被 B 站整体风控（ranking 都返回 -352）。**B 站服务器直连需要
  `.env` 配 `BILI_COOKIE`**（浏览器登录后的 Cookie，含 SESSDATA）即可恢复
  rank/creator/search 直连并拿到带音轨的完整视频；在此之前 B 站走上面的混合兜底路。

## 待办（plus）

- [x] 场景三跨平台搜索（发现层五平台 + 编排 + CLI + 混合路）
- [x] 批次总览 digest（四入口统一触发）
- [x] 报告导出 HTML/MD（CLI / Web / understand 三入口）
- [x] fast 模式、v_voucher 软风控退避
- [x] 新版 Forest 深色 Web 界面
...
## 2026-08-10 · 4090 迁移与统一回归

- SSH 切换到 RTX 4090 D（24564 MiB），确认原数据盘、四套模型和推理环境完整；
  同步 GitHub `main` 最新代码到 `/root/vspider`。
- 修复 Windows SFTP 后 Shell 脚本 CRLF 导致 Linux 无法执行：新增
  `.gitattributes`，统一规定 `*.sh` 使用 LF。
- 修复 `vllm_restart.sh` 没有真正杀掉旧 api_server/EngineCore 的问题：
  增加 pidfile、进程组清理和旧实例兜底匹配。
- vLLM 显存预算改为按显存自适应：24G 卡默认 0.75（18378 MiB），
  12G 卡保持 0.85；允许环境变量覆盖。
- Qwen3-8B 与 SenseVoice(cuda) 同卡实测 2/2、20.6s，无 OOM；
  同语料 E11 全 CPU 为 131.8s，约 6.4× 加速。
- 四平台场景一语料、四平台场景二语料、B站搜索与实时榜单共 20 个不同视频，
  GPU 理解链路 20/20；digest、SQLite、HTML 报告均成功。
- Web 首页、历史详情、HTML/Markdown 报告端点均 HTTP 200。
- 本机实时采集与下载首次为 4/5；快手强制重扫后保存 12 项 Cookie，
  榜单 2 条、下载 1 条（2.7 MB）、创作者作品 2 条均成功，最终达到
  五平台实时发现 5/5、下载 5/5。该创作者 `today` 为 0 条是当天无投稿。
- 小红书 fast A/B 发现“ASR 字数够但内容不充分”的反例；无标题/占位标题时
  fast 阈值改为两倍，保留 OCR 兜底。修后同批 2/2、22.4s，错误的“主题不明”
  被纠正为“热门舞蹈合集”。
- CLI 默认异常改为单行可行动提示，`--verbose` 才显示完整堆栈，避免演示时
  把平台 412 风控渲染成整屏 Python traceback。

### 五平台统一验收矩阵

- 榜单场景严格前5：五平台共25条，采集下载25/25、GPU理解25/25。
  B站新增 `--max-duration 0` 后原榜首107分钟视频也完整处理；严格批次715.2s，
  其中下载榜首占663.5s。默认1800秒保护保留供日常演示。
- 创作者场景：B站今日1条、微博今日5条完整GPU跑通；抖音/快手/小红书选定账号
  今日均0条，空结果写清单并正常退出；三个平台最近作品各2条GPU 6/6。
- 修复小红书 `--today`：创作者列表不带时间时从note_id前8位解析Unix秒时间戳，
  避免历史视频因时间未知混入今日结果。
- B站匿名空间接口连续5次HTTP 412；新增 `login_bili.py`，扫码保存19项Cookie后，
  同一作者今日作品接口1.63s返回并完成GPU理解。
- 搜索场景统一关键词“人工智能”：五平台各2条，采集下载10/10、GPU理解10/10；
  抖音无头模式返回空，有头浏览器立即成功，验收说明固定加 `--show-browser`。
- Web Plus复核：GPU 2/2、SSE事件完整、digest返回并落库、HTML/MD报告均200。
- 断点续跑命中69条，0.2s跳过；修复“全部跳过却退出1”的自动验收误判。
- 交付复现：新增 `setup_local.py` 固定MediaCrawler提交并安装依赖；
  新增 `preflight.py`，本机采集侧和4090理解侧实测全部通过。
- 本轮榜单25 + 创作者12 + 搜索10，共47条GPU结果，47/47成功。
