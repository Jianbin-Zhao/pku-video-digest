"""把一条视频的多路信息融合成给 LLM 的上下文。

只喂语音转写是不够的。短视频里有相当大的比例（小红书尤其明显）
根本没有人声，全靠画面、硬字幕和 BGM 表达，纯 ASR 会拿到空字符串。
所以这里把五路信号拼在一起：

    标题 / 话题标签 / 简介   平台侧的结构化元数据，最可靠
    语音转写                 有人声时信息量最大
    关键帧 OCR               无人声视频的主要信息来源，也能补回 ASR 漏掉的专有名词
    高赞评论                 观众视角的语义补充，常常直接点出视频的槽点或笑点
    互动数据                 判断这条内容为什么会上榜

每一路都设独立的字符预算，避免某一路（通常是 OCR 的重复字幕）
把上下文挤满，导致更可靠的标题和转写被截断。
"""
